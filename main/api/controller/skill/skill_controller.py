...
from ....api.serializer.Skill.skill_serializer import CreateSkillSerializer, GetSkillSerializer
...
class SkillController(ViewSet):
    ...
    @extend_schema(
        tags=['Skill'],
        responses={200: GetSkillSerializer},
    )
    def get(self, request: Request):
        user_skills = self.skill_logic.get_user_skill(user=request.user)

        serializer = GetSkillSerializer(user_skills, many=True)
